...
El técnico inicia el proceso de registro oprimiendo el botón **"Provisioning"** en el dashboard de Node-RED. Al ejecutarse, el sistema genera el ID del dispositivo a partir de la MAC address de `wlan0` y solicita los certificados a AWS IoT Core usando la plantilla `IbisaTemplate`:

```
Certificados guardados en: /home/pi/certificados/
```

Una vez completado el registro, Node-RED se reinicia para aplicar la nueva configuración.
